...
async def home():
    ...
    page = page.replace("hooks=s.discord_webhooks||[];draw()", "hooks=s.discord_webhooks||[];document.querySelectorAll('#discordRegions input').forEach(e=>{e.checked=(s.discord_regions||['combined','SEA']).includes(e.value);e.onchange=()=>save()});draw()", 1)
